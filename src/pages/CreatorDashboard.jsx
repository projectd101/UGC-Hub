...
export default function CreatorDashboard() {
  ...
  async function setIntroClip(video) {
    ...
    const { data, error } = await supabase
      .from("creators")
      .update({ intro_video_id: video.id })
      .eq("id", creator.id)
      .select()
      .single();
      ...
      <div className={styles.appArea}>
        ...
        <main className={styles.main}>
          <div className={styles.pageIntro}>
            <div><span className={styles.panelEyebrow}>Creator studio</span><h1 className={styles.title}>Welcome back, {creator?.display_name || "Creator"}</h1><p className={styles.subtitle}>Here's what's happening with your content today.</p></div>
          </div>
